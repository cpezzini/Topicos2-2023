...
def index():
    ...
    try:
        ...
        datos_bitacora = {
            ...
            "respuesta_riesgo_cardiaco": datos_respuesta_riesgo_cardiaco,  
...
@app.route('/llamar_actualizar_suscripcion', methods=['PUT'])
def llamar_actualizar_suscripcion():
    try:
        user = request.json.get('user')
        password = request.json.get('password')
        nuevo_tipo_suscripcion = request.json.get('tipo_suscripcion')

        if nuevo_tipo_suscripcion not in ["Premium", "Freemium"]:
            return jsonify({"error": "Tipo de suscripción no válido"}), 400

        url_actualizar_suscripcion = f"http://localhost:5005/actualizar_suscripcion"
        data = {"user": user,"password": password,"tipo_suscripcion": nuevo_tipo_suscripcion}
        headers = {'Content-Type': 'application/json'}
        respuesta_actualizar_suscripcion = requests.put(url_actualizar_suscripcion, json=data, headers=headers)
        respuesta_actualizar_suscripcion.raise_for_status()

        return jsonify({"message": "Solicitud para actualizar suscripción enviada correctamente"}), 200

    except requests.RequestException as e:
        return jsonify({"error": f"Error en la solicitud para actualizar suscripción: {str(e)}"}), 500
